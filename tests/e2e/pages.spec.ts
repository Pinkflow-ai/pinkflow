...
test('pricing publishes the Gateway credit contract without implying checkout is live', async ({ page }) => {
  ...
  await expect(page.getByText('$11.06', { exact: true }).first()).toBeVisible();
  await expect(page.getByText('$10.00 API usage', { exact: true }).first()).toBeVisible();
  ...
  await expect(page.getByText('$0.008 / call', { exact: true })).toBeVisible();
  await expect(page.getByText('Invoice & receipt extraction')).toBeVisible();
  await expect(page.getByText('50 credits', { exact: true })).toBeVisible();
  await expect(page.getByText('$0.050 / call', { exact: true })).toBeVisible();
  await expect(page.getByText('Direct or high-volume provider plans may have lower unit prices.')).toBeVisible();
  await expect(page.getByText(/25 free routes and 9 paid routes implemented in the developer-preview catalog/)).toBeVisible();
  await expect(page.getByText('9 paid routes')).toBeVisible();
  await expect(page.getByText(/OpenAPI 3\.1 contract.*TypeScript SDK.*Python SDK.*MCP stdio adapter/)).toBeVisible();
  await expect(page.getByText(/include the standard Paddle fee so the full API usage value reaches the account/)).toBeVisible();
  await expect(page.getByText(/100,000 credits cumulatively meets the permanent MCP unlock threshold/)).toBeVisible();
